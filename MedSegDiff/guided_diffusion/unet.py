...
class FFParser(nn.Module):
    def __init__(self, dim, h=320, w=320):
        super().__init__()
    ...
    def forward(self, x, spatial_size=None):
        ...
        x = torch.fft.ifftn(x, s=(H, W), dim=(2, 3), norm='ortho')
...
class UNetModel_v1preview(nn.Module):
    ...
    def forward(self, x, timesteps, y=None):
        ...
        for ind, module in enumerate(self.input_blocks):
            ...
            hs.append(h)
        # uemb is 1,512,10,10
        uemb, cal = self.highway_forward(c, [hs[3],hs[6],hs[9],hs[11]])
        uemb = F.interpolate(uemb, size=(h.shape[2],h.shape[3]))
        h = h + uemb
...
class Generic_UNet(SegmentationNetwork):
    ...
    def __init__(self, input_channels, base_num_features, num_classes, num_pool, num_conv_per_stage=2,
        ...
        for d in range(num_pool):
            ...
            if d < num_pool -1 and self.highway:
                self.conv_trans_blocks_a.append(conv_nd(2, int(d + 1) * 128, 2 **(d+5), 1))
                self.conv_trans_blocks_b.append(conv_nd(2, 2 **(d+5), 1, 1))
            if d != num_pool - 1 and self.highway:
                if d==3:
                    self.ffparser.append(FFParser(output_features, 320 // (2 **(d)), 320 // (2 **(d))))
                    # self.ffparser.append(FFParser(output_features,   // (2 **(d)), 160 // (2 **(d))))

                else:
                    self.ffparser.append(FFParser(output_features, 320 // (2 ** (d + 1)), 320 // (2 ** (d + 1))))
                    # self.ffparser.append(FFParser(output_features, 160 // (2 ** (d + 1)), 160 // (2 ** (d + 1))))
    ...
    def forward(self, x, hs = None):
        ...
        for d in range(len(self.conv_blocks_context) - 1):
            x = self.conv_blocks_context[d](x.float().cuda())
            skips.append(x)
            if not self.convolutional_pooling:
                x = self.td[d](x)
            if hs:
                h = hs.pop(0)
                ddims = h.size(1)
                h = self.conv_trans_blocks_a[d](h.cuda())
                h = self.ffparser[d](h)
                ha = self.conv_trans_blocks_b[d](h.float().cuda())
                ha = F.interpolate(ha,size=(x.shape[2],x.shape[3]))
                hb = th.mean(h,(2,3))
                hb = hb[:,:,None,None]
                x = x * ha * hb
